...
from config import CURRENCY_REGION_MAP, FX_RATES_TO_USD
...
if df_ns_session is None:
    ...
    if _tm_actuals.empty:
        ...
        if "currency" in _tm_actuals.columns:
            from config import get_fx_rate
...
if df_ns_session is not None:
    ...
    from config import get_fx_rate as _gfx
    def _to_usd_det(row):
...
with pd.ExcelWriter(_buf, engine="xlsxwriter") as _xl:
    ...
    if not slices.empty:
        from config import RECONCILE_IMPL_SKU as _RIMPL
...
try:
    ...
    if not slices.empty and "notes" in slices.columns:
        from config import RECONCILE_IMPL_SKU as _RDASH
